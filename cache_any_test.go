...
func TestCacheTimes(t *testing.T) {
	...
	tc := New[string, any](50*time.Millisecond, 1*time.Millisecond)
	tc.Set("a", 1, DefaultExpiration)
...
func TestOnEvicted(t *testing.T) {
	...
	works := false
	tc.OnEvicted(func(k string, v interface{}, hit int) {
		if k == "foo" && v.(int) == 3 {
			works = true
		}
...
func testFillAndSerialize(t *testing.T, tc *Any[string, any]) {
